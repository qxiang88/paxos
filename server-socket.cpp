...
void* AcceptConnections(void* _S) {
    ...
    while (1) {
        ...
        if (incoming_port == S->get_master_port()) { // incoming connection from master_port
            ...
            int process_id = S->IsServerPaxosPort(incoming_port);
            if (process_id != -1) { //incoming connection from paxos port of a server
                S->set_server_paxos_fd(process_id, new_fd);
            } else {
 ...
 */
bool Server::ConnectToServer(const int server_id) {
    if (get_server_paxos_fd(server_id) != -1) return true;
    ...
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_flags = AI_PASSIVE; // use my IP
    if ((rv = getaddrinfo(NULL, std::to_string(get_server_paxos_port(get_pid())).c_str(),
                          &hints, &clientinfo)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        exit (1);
    }
